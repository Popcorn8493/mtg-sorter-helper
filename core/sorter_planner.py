...
class SorterPlanner:
    ...
    def create_sorting_plan(
        self, cards: List[Card], sort_order: List[str]
    ) -> List[SortGroup]:
        ...
        for group_name, group_cards in grouped_cards.items():
            ...
            unsorted_count = sum(
                card.quantity - card.sorted_count for card in group_cards
            )

            sort_group = SortGroup(
                group_name=group_name, count=unsorted_count, cards=group_cards
            )
            sort_group.total_count = total_count
            sort_group.unsorted_count = unsorted_count

            if len(sort_order) > 1:
                sort_group.sub_groups = self.create_sorting_plan(
                    group_cards, sort_order[1:]
                )
    ...
    def create_set_letter_plan(
        ...
        threshold: int = 20,
    ) -> Tuple[List[SortGroup], Dict[str, str]]:
        if optimal_grouping:
    ...
    def _create_grouped_letter_plan(
        self, cards: List[Card], threshold: int
    ) -> Tuple[List[SortGroup], Dict[str, str]]:
        ...
        for i, letter in enumerate(letters):
            ...
            if 0 < count < threshold:
                ...
                next_letter_small = (
                    i < 25 and 0 < letter_counts.get(letters[i + 1], 0) < threshold
                )
        ...
        piles = collections.defaultdict(
            lambda: {"cards": [], "total": 0, "unsorted": 0}
        )
        for card in cards:
            name = getattr(card, "name", "")
            if name and name != "N/A":
    ...
    def _create_optimal_letter_plan(
        self, cards: List[Card], threshold: int
    ) -> Tuple[List[SortGroup], Dict[str, str]]:
        import string
        ...
        letter_counts = [
            (letter, raw_letter_totals.get(letter, 0))
    ...
    def _create_simple_letter_plan(
    ...
    def _group_cards_by_criterion(
        self, cards: List[Card], criterion: str
    ) -> Dict[str, List[Card]]:
    ...
    def get_cards_at_path(
        self, sort_groups: List[SortGroup], path: List[str]
    ) -> List[Card]:
        """
