...
class SortableTreeWidgetItem(QTreeWidgetItem):
    ...
    def __lt__(self, other):
        ...
        - Column 1 (Count): Sorts numerically, treating empty strings as
          smallest.
        ...
        # For columns with "key: value" format, sort by value for more natural
        # sorting.
...
class NavigableTreeWidget(QTreeWidget):
    ...
    def _block_signals(self):
        """Block signals with reference counting"""
    ...
    def _unblock_signals(self):
        """Unblock signals with reference counting"""
    ...
    def cleanup(self):
        ...
        # Block signals to prevent any last-minute events on a widget that is
        # being destroyed. Qt's object ownership model will handle the actual
        # disconnection of signals when this widget is deleted by its parent or
        # by deleteLater().
        self.blockSignals(True)
    ...
    def mouseDoubleClickEvent(self, event: QMouseEvent | None):
        ...
        try:
            ...
            if item and event.button() == Qt.MouseButton.LeftButton:
                ...
                if (
                    event.pos().x() >= checkbox_start
                    and event.pos().x() <= checkbox_end
                ):
    ...
    def keyPressEvent(self, event: QKeyEvent | None):
        """FIXED: Safe keyboard event handling by dispatching to specialized
        handlers."""
        if self._is_destroyed or self._in_operation or not event:
            return
        ...
        try:
            ...
            # Sequentially try handlers. If a handler processes the key, accept
            # the event and return.
            if (
                self._handle_item_action_keys(event)
                or self._handle_navigation_keys(event)
                or self._handle_selection_keys(event)
            ):
                event.accept()
                return

            # If no custom handler processed the key, pass it to the base
            # class.
    ...
    def _handle_item_action_keys(self, event: QKeyEvent | None) -> bool:
        """Handles key presses that perform actions on the current/selected
        item(s)."""
        if not event:
            return False
        key = event.key()
        ...
        if key in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
            ...
            if selected_items:
                self._emit_signal(
                    lambda: self.markAsortedRequested.emit(selected_items)
                )
                return True
    ...
    def _emit_signal(self, emit_func):
        """Emit signals with deferred execution"""
        if self._is_destroyed:
            return
    ...
    def mousePressEvent(self, event: QMouseEvent | None):
        """FIXED: Safe mouse press handling that allows itemClicked signal
        emission"""
        ...
        try:
            ...
            if event.button() == Qt.MouseButton.LeftButton:
                ...
                if item:
                    ...
                    # Check if click is in the checkbox area (first 20 pixels
                    # from the item content). Calculate the depth of the item
                    # in
                    # the tree
                    depth = 0
                    parent = item.parent()
                    ...
                    if (
                        ...
                        # This is a checkbox click - handle it and completely
                        # prevent default behavior
                        self._handle_checkbox_click(item)
                        event.accept()
                        # Don't call super() to prevent any default tree widget
                        # behavior
                        return

            # Don't set _in_operation flag here as it blocks Qt's signal
            # emission
            # Call parent first to allow normal Qt event processing and signal
            # emission
            super().mousePressEvent(event)

            # Update tooltip based on what was clicked (deferred to avoid
            # interference)
            if event.button() == Qt.MouseButton.LeftButton:
                item = self.itemAt(event.pos())
                if item:
                    # Queue the details showing with longer delay to avoid
    ...
    def _populate_tree_progressively(
        self, nodes, parent_item=None, chunk_size=100, on_finished=None
    ):
        ...
        print(
            f"DEBUG: Processing chunk of {len(chunk)} nodes, "
            f"{len(remaining_nodes)} remaining"
        )
            ...
            for i, node in enumerate(chunk):
                try:
                    tree_item = SortableTreeWidgetItem(
                        parent_item, [node.group_name, str(node.count)]
                    )
                    ...
                    if hasattr(node, "cards") and node.cards:
                        ...
                        if all_sorted:
                            ...
                            while parent_widget and not hasattr(
                                parent_widget, "show_sorted_check"
                            ):
                                parent_widget = parent_widget.parent()

                            if parent_widget and hasattr(
                                parent_widget, "show_sorted_check"
                            ):
                                show_sorted = (
                                    parent_widget.show_sorted_check.isChecked()
                                )
                                if not show_sorted:
        ...
        if remaining_nodes:
            print(
                f"DEBUG: Scheduling next chunk "
                f"({len(remaining_nodes)} nodes remaining)"
            )
            QTimer.singleShot(
                0,
                lambda: self._populate_tree_progressively(
                    remaining_nodes, parent_item, chunk_size, on_finished
                ),
            )
        else:
            ...
            def final_actions():
                ...
                try:
                    ...
                    self.sortByColumn(
                        self.sortColumn(), self.header().sortIndicatorOrder()
                    )
    ...
    def _on_item_changed(self, item, column):
        ...
        try:
            ...
            if hasattr(item, "checkState"):
                ...
                QTimer.singleShot(
                    0, lambda: self.itemSortedToggled.emit(item, is_sorted)
                )
