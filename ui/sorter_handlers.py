# ui/sorter_handlers.py - Event handlers and UI logic for sorter tab
...
class SorterHandlers:
    """Handles UI events and interactions for the sorter tab"""
        ...
        try:
            ...
            if isinstance(current_widget, SetSorterView):
                ...
                if cards_in_pile:
                    ...
                    self.parent.navigate_to_level(
                        self.parent.results_stack.currentIndex()
                    )
                    ...
                    )  # This is the Name level (individual cards)
                    self.parent.create_new_view(cards_in_pile, final_level)
                    self.parent.update_button_visibility()
                    return

            if (
                0 <= current_level < len(self.parent.sort_order)
                and self.parent.sort_order[current_level] == "Name"
            ):
            ...
            self.parent.show_status_message(
                f"Drilling down into '{item.text(0).split(': ')[-1]}'...",
                2000,
                style="info",
            )
            ...
            self.parent.handle_ui_error(
                "handle_item_click",
                e,
                additional_context=f"item: {item.text(0) if item else 'None'}, level: {next_level}",
            )
    ...
    def drill_down(self, item: QTreeWidgetItem, next_level: int):
        ...
        try:
            ...
            if not self.parent.sort_order:
                self.parent.show_status_message(
                    "Configuration error: Sort criteria not available.",
                    3000,
                    style="error",
                )
                return
            ...
            if next_level > len(self.parent.sort_order):
                self.parent.show_status_message(
                    "Cannot drill down further - no more sort criteria available.",
                    3000,
                    style="warning",
                )
                return
            ...
            if 0 <= current_level_index < len(self.parent.sort_order):
                ...
                next_criterion = (
                    self.parent.sort_order[next_level]
                    if next_level < len(self.parent.sort_order)
                    else None
                )
                ...
                if current_criterion == "Set" and next_criterion == "First Letter":
                    ...
                    if not cards_in_set:
                        self.parent.show_status_message(
                            "No cards found in selected set.", 2000, style="warning"
                        )
                        return

                    self.parent.navigate_to_level(current_level_index)
                    breadcrumb_text = item.text(0).split(": ")[-1]
                    self.parent.add_breadcrumb(
                        f"{breadcrumb_text} (Letter Sort)", next_level
                    )
            ...
            if not cards_in_group:
                self.parent.show_status_message(
                    "No cards found in selected group.", 2000, style="warning"
                )
                return
            ...
            try:
                ...
                traceback.print_exc()
                self.parent.show_status_message(
                    "Navigation error occurred.", 3000, style="error"
                )
                return
            ...
            try:
                ...
                traceback.print_exc()
                self.parent.show_status_message(
                    "Failed to create view for next level.", 3000, style="error"
                )
                return
            ...
            self.parent.handle_ui_error(
                "drill_down",
                e,
                additional_context=f"item: {item.text(0) if item else 'None'}, level: {next_level}, navigating: {self.parent._is_navigating}",
            )
        finally:
    ...
    def on_item_sorted_toggled(self, item: QTreeWidgetItem, is_sorted: bool):
        ...
        try:
            ...
            self.parent.show_status_message(
                f" Marked '{item.text(0)}' as {action}.", style="success"
            )
    ...
    def on_mark_group_button_clicked(self):
        ...
        if (
            QMessageBox.question(
                ...
                f"Mark {total_cards_affected} cards as sorted?",
            )
            ...
            self.parent.show_status_message(
                f"Marked {len(selected_items)} groups as sorted ({total_cards_affected} cards)",
                style="success",
            )
            self.parent.project_modified.emit()
    ...
    def mark_item_as_sorted(self, item: QTreeWidgetItem):
        ...
        if is_already_sorted:
            self.parent.show_status_message(
                f" Group '{item.text(0)}' marked as UNSORTED.", style="warning"
            )
        else:
            self.parent.show_status_message(
                f" Group '{item.text(0)}' marked as SORTED.", style="success"
            )
    ...
    def filter_current_view(self, text: str):
        ...
        elif isinstance(current_widget, SetSorterView) and hasattr(
            current_widget, "tree"
        ):
        ...
        if tree_to_filter:
            iterator = QTreeWidgetItemIterator(
                tree_to_filter, QTreeWidgetItemIterator.IteratorFlag.All
            )
    ...
    def update_button_visibility(self, *args):
        ...
        is_normal_view = isinstance(
            self.parent.results_stack.currentWidget(), NavigableTreeWidget
        )
    ...
    def add_criterion(self, item):
        """Add criterion to selected list"""
        self.parent.selected_list.addItem(
            self.parent.available_list.takeItem(self.parent.available_list.row(item))
        )
        self.parent.project_modified.emit()

    def remove_criterion(self, item):
        """Remove criterion from selected list"""
        self.parent.available_list.addItem(
            self.parent.selected_list.takeItem(self.parent.selected_list.row(item))
        )
        self.parent.project_modified.emit()

    def update_progress(self, value, total):
        """Update progress bar and file label"""
