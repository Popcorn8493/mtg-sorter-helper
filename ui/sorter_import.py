# ui/sorter_import.py - CSV import and data processing for sorter tab
...
class SorterImport:
    """Handles CSV import and data processing for the sorter tab"""
    ...
    def import_csv(self, filepath=None):
        ...
        if not filepath:
            ...
            settings.setValue(
                "sorter/lastImportDir", str(pathlib.Path(filepath).parent)
            )
        ...
        try:
            ...
            if not file_path.is_file():
                QMessageBox.critical(
                    self.parent, "Invalid File", f"'{filepath}' is not a valid file."
                )
                return
            ...
            self.parent.handle_file_error(
                "file access",
                e,
                additional_context=f"filepath: {filepath}, exists: {file_path.exists() if 'file_path' in locals() else 'Unknown'}",
            )
            QMessageBox.critical(
                self.parent, "File Access Error", f"Unable to read file: {e}"
            )
            return
        ...
        self.parent.operation_started.emit(
    ...
    def on_import_finished(self, cards: List[Card]):
        ...
        try:
            ...
            self.parent.file_label.setText(
                f"Loaded {unique_count:,} unique cards ({total_count:,} total)"
            )
            ...
            self.parent.start_background_image_cache(cards)
            ...
            self.parent.handle_critical_error(
                "import process",
                e,
                additional_context=f"cards_count: {len(self.parent.all_cards) if self.parent.all_cards else 0}, loading: {self.is_loading}",
            )
            self.is_loading = False
    ...
    def clear_project(self, prompt=True):
        ...
        self.parent.show_status_message(
            "Project cleared. Import a new CSV to begin.", 5000, style="info"
        )
        self.parent.project_modified.emit()
    ...
    def reset_sort_progress(self):
        ...
        if not self.parent.all_cards:
            QMessageBox.information(
                self.parent, "No Collection", "No collection is loaded."
            )
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self.parent.show_status_message(
                "All sorting progress has been reset.", 3000, style="info"
            )
            self.parent.project_modified.emit()
