...
class SorterTabUi:
    """Handles the creation and layout of all UI elements for the Sorter Tab using factory pattern."""
    ...
    def setup_ui(self, main_layout: QVBoxLayout):
        """
        Creates and adds the main UI sections to the provided layout using factory pattern.

        Args:
            main_layout: The main QVBoxLayout of the parent tab.
        """
        # Create layout widgets that need special handling
        self._create_layout_widgets()

        # Create all widgets using the factory
        self._create_all_widgets()

        # Create UI sections with the created widgets
    ...
    def _create_all_widgets(self):
        """Create all widgets using the factory pattern."""
        ...
        for section_name, configs in all_configs.items():
            for config in configs:
                # Skip breadcrumb_layout as it's handled specially
                if config.name == "breadcrumb_layout":
                    continue

                # Set up connections for the tab widget
                self._setup_widget_connections(config)

                # Create the widget
                widget = self.widget_factory.create_widget(config)

                # Assign to tab_widget attributes for backward compatibility
                setattr(self.tab_widget, config.name, widget)
    
    def _setup_widget_connections(self, config):
        """Set up signal connections for widgets."""
        connections_map = {
            "import_button": {"clicked": self.tab_widget.import_csv},
            "reset_progress_button": {
                "clicked": self.tab_widget.reset_sort_progress
            },
            "available_list": {
                "itemDoubleClicked": self.tab_widget.add_criterion
            },
            "selected_list": {
                "itemDoubleClicked": self.tab_widget.remove_criterion
            },
            "run_button": {
                "clicked": self.tab_widget.start_new_plan_generation
            },
            "show_sorted_check": {
                "stateChanged": self.tab_widget.on_show_sorted_toggled
            },
            "mark_sorted_button": {
                "clicked": self.tab_widget.on_mark_group_button_clicked
            },
            "export_button": {
                "clicked": self.tab_widget.export_current_view
            },
            "filter_edit": {
                "textChanged": self.tab_widget.filter_current_view
            },
            "fetch_image_button": {
                "clicked": self.tab_widget.on_fetch_image_clicked
            }
        }
    ...
    def _create_layout_widgets(self):
        """Create layout widgets that need special handling."""
        # Create breadcrumb layout as a widget container
        ...
        # Assign the layout to the tab widget
    ...
    def _create_import_section(self, layout: QVBoxLayout):
        """Create the import section using factory-created widgets."""
    ...
    def _create_options_section(self, layout: QVBoxLayout):
        """Create the options section using factory-created widgets."""
        ...
        sort_order_group.setToolTip(
            "Define the hierarchy for organizing your cards. Double-click to move "
            "criteria between lists. Drag-and-drop within the 'Selected' list to "
            "reorder.")
        ...
        set_plan_group.setToolTip(
            "Options for optimizing letter-based sorting within sets")
    ...
    def _create_run_section(self, layout: QVBoxLayout):
        """Create the run section using factory-created widgets."""
    ...
    def _create_results_section(self, layout: QVBoxLayout):
        """Create the results section using factory-created widgets."""
