...
def create_splash_screen():
    try:
        ...
        painter.drawText(
            splash_pixmap.rect(), Qt.AlignmentFlag.AlignCenter, "MTG Toolkit\nEnhanced"
        )
...
def check_system_requirements():
    ...
    try:
        ...
        if free_space < required_space:
            issues.append(
                f"Low disk space: {free_space // (1024 * 1024)} MB available, 1GB recommended"
            )
    except Exception:
        pass
    ...
    try:
        ...
        issues.append(
            "Could not connect to Scryfall API. Check your internet connection."
        )
...
Examples:
  ...
  %(prog)s --import file.csv        # Import collection on startup
    ...
    parser.add_argument(
        "--version", action="version", version="MTG Toolkit Enhanced v1.0"
    )
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument(
        "--theme", choices=["dark", "light"], help="Start with specified theme"
    )
    parser.add_argument(
        "--clear-cache", action="store_true", help="Clear all cache on startup"
    )
    parser.add_argument(
        "--import",
        dest="import_file",
    ...
    try:
                ...
                if api.clear_cache():
                    ...
                    try:
                        QMessageBox.information(
                            None, "Cache Cleared", "Application cache has been cleared."
                        )
                    except:
                    ...
                    try:
                        QMessageBox.warning(
                            None, "Cache Error", "Could not clear application cache."
                        )
                    except:
            ...
            try:
                ...
                if issues:
                    ...
                    try:
                        ...
                        print(
                            f"Warning: System requirement issues detected:\n{issue_str}"
                        )
            except Exception as e:
        ...
        if args.import_file and main_window:
            try:
                if splash:
                    splash.showMessage(
                        f"Importing {os.path.basename(args.import_file)}...",
                        Qt.AlignmentFlag.AlignBottom | Qt.AlignmentFlag.AlignCenter,
                        QColor("white"),
                    )
                QTimer.singleShot(
                    100,
                    lambda: main_window.sorter_tab.import_csv(
