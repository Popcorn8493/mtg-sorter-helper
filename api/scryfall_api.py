...
class ScryfallAPI:
    ...
    def fetch_card_collection(self, identifiers: List[Dict[str, str]]) -> List[Dict[str, Any]]:
        """
        Fetches a collection of cards using the Scryfall API's /cards/collection endpoint with memory safety.
        ...
        for i in range(0, len(identifiers), chunk_size):
            ...
            try:
                time.sleep(0.1)  # Rate limiting to prevent API overload
    ...
    def fetch_image(self, image_uri: str, scryfall_id: str) -> bytes:
        ...
        if not (image_uri.startswith('http://') or image_uri.startswith('https://')):
            raise MTGAPIError(
                f"Invalid image URI format: {image_uri}",
                "validation_error"
            )
        ...
        try:
            ...
            for chunk in response.iter_content(chunk_size=8192):
                if total_size + len(chunk) > max_size:
                    raise MTGAPIError(
                        f"Image too large for {scryfall_id}",
                        "size_limit_exceeded",
            ...
            traceback.print_exc()
            raise MTGAPIError(
                f"Unexpected error downloading image for {scryfall_id}: {str(e)}",
                "unexpected_error",
                {"image_uri": image_uri, "original_error": str(e)}
            )
